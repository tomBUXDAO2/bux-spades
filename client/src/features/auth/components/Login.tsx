...
const Login: React.FC = () => {
  ...
  const handleFacebookLogin = () => {
    ...
    const facebookAuthUrl = `https://www.facebook.com/v18.0/dialog/oauth?client_id=${appId}&redirect_uri=${redirectUri}&response_type=code&scope=${scope}`;
    
    try {
      window.location.href = facebookAuthUrl;
    } catch (error) {
      console.error('Facebook login redirect failed:', error);
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center space-y-4">
          <p className="text-slate-300 text-lg">
            Login to BUX Spades with Discord or Facebook
          </p>
        ...
        <div className="mt-8 space-y-3">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full inline-flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
