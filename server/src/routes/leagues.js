  ...
  try {
    ...
    try {
      ...
      const announcementPushPayload = {
        ...
        data: {
          type: 'league_announcement_created',
  ...
  try {
    ...
    try {
      ...
      for (const reg of tournament?.registrations || []) {
        if (reg.userId) recipientIds.add(reg.userId);
        if (reg.partnerId) recipientIds.add(reg.partnerId);
      }
