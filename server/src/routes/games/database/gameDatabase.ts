 ...
 */
export async function ensureGamePlayerRecords(game: Game): Promise<void> {
  try {
    console.log('[DATABASE] Ensuring GamePlayer records exist for game:', game.id);
    
    // Check if GamePlayer records exist
    const existingPlayers = await prisma.gamePlayer.count({ where: { gameId: game.id } });
    if (existingPlayers === 0) {
      console.log('[DATABASE] No GamePlayer records found, creating them...');
      ...
      for (let i = 0; i < 4; i++) {
        ...
        try {
          ...
          await prisma.gamePlayer.create({
            data: {
              id: `player_${game.id}_${i}_${Date.now()}`,
              gameId: game.id,
              userId: userId,
              position: i,
              team: team,
      ...
      console.log(`[DATABASE] GamePlayer records already exist (${existingPlayers} found)`);
    }
  } catch (error) {
    console.error('[DATABASE] Failed to ensure GamePlayer records:', error);
    throw error;
  }
}
