...
io.on('connection', (socket: AuthenticatedSocket) => {
  ...
  socket.on('play_start', ({ gameId }) => {
    ...
    if (currentPlayer && currentPlayer.type === 'bot') {
      ...
      setTimeout(() => {
              console.log("[HAND TRANSITION DEBUG] setTimeout callback executing for game:", game.id);
      ...
      if (game.play.trickNumber === 13) {
        ...
        if (game.gameMode === 'SOLO') {
          ...
          if (shouldEndGame && winningTeam) {
            ...
            console.log("[HAND TRANSITION DEBUG] Game state before setTimeout:", {
              id: game.id,
              status: game.status,
              dealerIndex: game.dealerIndex,
              players: game.players.map(p => p ? p.username : null)
            });
            setTimeout(() => {
              console.log("[HAND TRANSITION DEBUG] setTimeout callback executing for game:", game.id);
              ...
              io.to(game.id).emit("game_update", enrichGameForClient(game));
                          console.log("[HAND TRANSITION DEBUG] setTimeout callback completed for game:", game.id);
            }, 12000); // 12 second delay to match frontend timer
          }
        }
      }
